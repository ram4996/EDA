...
void ProblemD::run(){
	...
	auto t_findpath_start = Clock::now();
	find_path();
	auto t_findpath_end = Clock::now();
	cout<<"Find all paths druation: "<<std::chrono::duration<double>(t_findpath_end-t_findpath_start).count()
		<< " seconds" << std::endl;
	...
	cout<<"A single input vector process druation: "<<std::chrono::duration<double>(t_sim_end-t_sim_start).count()
		<< " seconds" << std::endl;
}
...
vector<pair<int, string>> ProblemD::split_(string str, char *delim, bool _Makeinout, bool _simu){ 
	...
	*/
	...
	split.emplace_back(make_pair(Unit, otherPath_pchn));
...
bool ProblemD::find_next(vector<pair<int, int>> &pos, int row, int col, string tar){ 
	...
	*	if found then store position as pos.emplace_back(make_pair(rowPosition, colPosition)), 
	...
	*/
	...
	for(itRow = parse.begin()+row; itRow != parse.end(); itRow++){		 
		...
		if(itCol != itRow->end()-1){
			...
			pos.emplace_back(make_pair(rowPosition, colPosition));
			found = true;
			break;
		}
	}
...
void ProblemD::find_path(){
	...
	*/
	...
	for(size_t i = 0; i < parse.size(); i++){ //find the path in U1~Uend
		for(size_t j = 2; j < parse[i].size()-1; j++){

			if(input_check(parse[i][j])){

				//cout<<"Is input:"<<parse[i][j]<<"in "<<i<<", "<<j<<endl;
				path.emplace_back("U"+to_string(i+1)+"/"+parse[i][j]);
				//path.emplace_back("U"+to_string(i+1)+"/");
				//cout<<"U"+to_string(i+1)+"/"+parse[i][j]<<endl;

				if( (parse[i][parse[i].size()-1])[0] == 'M'){

					//tar = parse[i][parse[i].size()-1];
					//path.emplace_back("U"+to_string(i+1)+"/"+tar); 
					path.emplace_back(parse[i][parse[i].size()-1]);
					...
					path.emplace_back("start"); //**It will be pop in !new_path  
					...
					while(!path.empty()){
						...
						if(find_next(pos, row, col, tar)){ 
							...
							path.emplace_back("U"+to_string(pos[0].first+1)+"/"+tar);
							row = pos[0].first; 
	...
	if(allpath.size() < 50)
		...
		cout<<"Total Path is too many to print out!"<<endl;
}
...
void ProblemD::parsefunc(){ 
	...
	*/
	...
	if(!fin){ cout<<"Fail to open file"<<endl; throw std::exception(); }
		...
		if(strline.empty()){ start_parse = true; }
			...
			while (pch != NULL){
				...
				if(strcmp(pch, ");") != 0 && strcmp(pch, "endmodule") != 0 && strcmp(pch, "(") != 0 ){	//We don't need such string
					...
					if(strpch[0] == 'U'){
						strpch = "U" + std::to_string(counter);
						counter++;
					}
					str.emplace_back(strpch);
				}
				...
				if(strcmp(pch, ");") == 0){ //");" means newline
					...
					for(size_t i = 0; i < str.size()-2; i++) 
						_int_sim.emplace_back(-1);
					sim.emplace_back(_int_sim);
					_int_sim.clear();

					parse.emplace_back(str);
					str.clear();
				}
			...
			while (pch != NULL){
				...
				if(strcmp(pch, "input") == 0){
					...
					_allinvec.emplace_back(_inout_vec);
					_inout_vec.clear();
				}else if(strcmp(pch, "output") == 0){

					pch = strtok(NULL, " ");
					_make_inout(pch, _inout_vec, true);
					_alloutvec.emplace_back(_inout_vec);
					_inout_vec.clear();
				}
				pch = strtok(NULL, " ");
			}
	...
	*/
	...
	for(size_t i = 0; i < parse.size(); i++){ // initial the input
		for(size_t j = 2; j < parse[i].size()-1; j++){
			...
			if(input_check(parse[i][j]) == true && (parse[i][j])[0] != 'M'){ //In problemD 'M' is output.
		...
		for(size_t i = 0; i < sim.size(); i++){			
			...
			if(check_1DrowReady(sim[i]) == true){
				...
				if( (parse[i][parse[i].size()-1])[0] == 'M'){ 
					...
					loop_max++; //print_vec(_alloutvec);
					split.clear();	
				}
				else{

					//Current output isn't Primary Out, need update table.
					//cout<<i+1<<" "<<parse[i][parse[i].size()-1]<<endl;
					SimTable_update(parse[i][parse[i].size()-1], lt_catch);
				}
			}//If END
		}//For loop END
	...
	//print result
	cout<<endl;
	cout<<"A single simulate result is: "<<endl;
